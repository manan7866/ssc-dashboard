...
export async function GET(req: NextRequest) {
  ...
  if (authHeader && authHeader.startsWith('Bearer ')) {
    ...
    if (!session || !session.accessToken) {
      console.log('No valid session found for user submissions access');
  ...
  if (!token) {
    console.log('No token available for user submissions access');
    return NextResponse.json({ message: 'Unauthorized' }, { status: 401 });
  }

  // Forward the request to the backend
  const BACKEND_URL = process.env.BACKEND_URL || 'https://abdulmanan04-ssc-dashboards.hf.space';
  const backendUrl = `${BACKEND_URL}/api/conferences/user/submissions`;
